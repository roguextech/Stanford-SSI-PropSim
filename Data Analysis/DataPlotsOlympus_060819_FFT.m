...
reactants =   [                                         ...
       CEA.Reactant('RP-1','Type','Fuel','T',DimVar(298,'K'),'Q',DimVar(0.1,'kg')), ...
       CEA.Reactant('N2O','Type','ox','T',DimVar(0,'K'), 'Q',DimVar(0.1,'kg')) ...
            ];       
        
% Find the OF ratio in time
m_dot_ox = m_ox/((length(burn_ind)/100));
OF_test = m_dot_ox./m_dot_fuel;

tcc = zeros(length(burn_ind), 1);

% Iterate across the OF ratio, chamber pressure
for i = burn_ind
%     test_p(i) = pcc(i);
    test_p = pcc(i)/psi_to_Pa;
    ...
    tcc(i) = data.Temperature(2).Value;
